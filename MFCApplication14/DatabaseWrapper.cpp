...
bool DatabaseWrapper::addPerson(string name, int identifier, Person personType)
{
	BasicExcel e;
	e.Load(PEOPLE_FILE);
...
bool DatabaseWrapper::linkPersonCourse(int identifier, Person personType, string courseId)
{
	BasicExcel excelFile;
...
bool DatabaseWrapper::setQuizMarks(string courseId, int identifier, int quizNumber, int marks)
{
	BasicExcel excelFile;
	excelFile.Load(COURSE_FILE);
	BasicExcelWorksheet *sheet = excelFile.GetWorksheet(courseId.c_str());
...
int DatabaseWrapper::getStudQuizMarks(string courseId, int identifier, int quizNumber)
{
	BasicExcel excelFile;
	excelFile.Load(COURSE_FILE);
	BasicExcelWorksheet *sheet = excelFile.GetWorksheet(courseId.c_str());
...
void DatabaseWrapper::getCourseStudents(string courseId, int *size, Student** students)
{
	Student* st[100];
	...
	if (!sheet || !sheet2) { students = NULL; return; }
	*size = 0;
	for (size_t i = 2; i < sheet->GetTotalRows(); i++)
	{
		...
		if (found == -1) { students = NULL; return; }
		st[*size] = new Student(sheet2->Cell(found, 1)->GetString(), (long long)identifier);
		(*size)++;
	}
...
void DatabaseWrapper::getCourses(int identifier, Person personType, int *size, Course** courses)
{
	BasicExcel excelFile;
	excelFile.Load(PEOPLE_FILE);
	BasicExcelWorksheet *sheet;
	...
	if (found == -1)
		...
		a = (char*)sheet->Cell(found, COURSE_COL)->GetString();
